...
export interface AuditEntry { action: string; entity: string; entityId?: string | number | null; entityLabel?: string | null; before?: unknown; after?: unknown; note?: string }
...
export class AuditClient {
  ...
  async record(client: Queryable, entry: AuditEntry): Promise<void> {
    ...
    const event = eventFromContext(this.source, EVENTS.audit.recorded, {
      ...
      actor: ctx?.actor ?? { id: 'system', name: 'system', kind: 'system' }, ip: ctx?.ip ?? null, at: new Date().toISOString(), service: this.source,
