import { getJurisdiction, MODULE_SETTING_DEFAULTS } from '@maritime/contracts';
import { SMART_PROGRAMME_START } from './inspection';
...
export function buildSettings(profile: string): WorldSetting[] {
  ...
  return [
    ...
    { key: 'billing', value: { taxName: j.tax.name, taxRate: j.tax.ratePct, taxRegistrationLabel: j.tax.registrationLabel, placeOfSupply: ae ? 'Abu Dhabi' : 'Maharashtra', serviceCode: '996751', currency: j.currency.code } },
    ...
    ...Object.entries(MODULE_SETTING_DEFAULTS).map(([k, v]) => ({ key: `module:${k}`, value: { ...v, ...(k === 'finance' ? { invoicePrefix: j.tax.invoicePrefix } : {}), ...(k === 'ops' ? { vcnPrefix: ae ? 'MAR' : 'REF' } : {}),
      // the Smart Inspection programme went live on a day the world knows; the KPIs are measured from it
      ...(k === 'inspect' ? { kpiProgrammeStart: SMART_PROGRAMME_START.toISOString().slice(0, 10) } : {}) } })),
  ];
}
