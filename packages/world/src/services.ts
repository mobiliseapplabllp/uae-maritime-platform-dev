import { getJurisdiction, VESSEL_STATUTORY_CERT_TYPES, COMPANY_LICENSE_TYPES, type SubjectKind, type RequestStatus, type DefinitionStatus } from '@maritime/contracts';
import { buildLookups } from './reference';
...
/** A select either carries inline options or names a Data Studio master (`lookup`); the runtime validates against its mirror of that master. */
export interface WorldServiceField { key: string; label: string; labelAr?: string; type: 'text' | 'number' | 'date' | 'select' | 'checkbox' | 'textarea'; options: string[]; lookup?: string; required: boolean; help: string }
...
type F = [string, string, string, WorldServiceField['type'], boolean, string[], string?]; // key, label, Arabic label, type, required, inline options, master the options come from
...
const SURVEY_FIELDS: F[] = [['surveyPort', 'Port at which the survey is to be held', 'ميناء إجراء المعاينة', 'text', true, []], ['surveyDate', 'Requested survey date', 'تاريخ المعاينة المطلوب', 'date', true, []], ['recognisedOrganisation', 'Recognised organisation acting for the flag', 'الهيئة المعترف بها', 'select', false, [], 'recognisedOrganisation']];
...
const ROWS: Row[] = [
  ...
  ['REG-AMENDMENT', 'Amendment of registry particulars', 'تعديل بيانات التسجيل', 1, 'Registration', 'VESSEL', '', 1000, 10000, 15, false, EVIDENCE.AMENDMENT.map((e) => e.label), [['amendmentTypes', 'Nature of the alteration', 'طبيعة التعديل', 'select', true, [], 'amendmentType']]],
  ['REG-DELETION', 'Closure of registry (deletion)', 'إغلاق التسجيل (الشطب)', 1, 'Registration', 'VESSEL', '', 500, 5000, 15, false, EVIDENCE.DELETION.map((e) => e.label), [['reason', 'Ground for closure', 'سبب الإغلاق', 'select', true, [], 'deletionReason'], ['newFlag', 'Receiving flag', 'العلم المستقبل', 'text', false, []]]],
  ...
  ['VESSEL-NAV-LIC', 'Navigation Licence — issue', 'رخصة الملاحة — إصدار', 1, 'Licensing', 'VESSEL', 'NAVIGATION_LICENCE', 2500, 25000, 10, false, ['Certificate of Registry', 'Insurance certificate', 'Class certificate'], [['voyageArea', 'Intended area of operation', 'منطقة التشغيل المقصودة', 'select', true, [], 'voyageArea'], ['startDate', 'Requested commencement', 'تاريخ البدء المطلوب', 'date', true, []]]],
  ['VESSEL-FOREIGN-PERMIT', 'Foreign Flag Vessel Permit — coastal trade', 'تصريح سفينة أجنبية — الملاحة الساحلية', 1, 'Licensing', 'VESSEL', 'FOREIGN_VESSEL_PERMIT', 5000, 50000, 7, false, ['Flag state certificate', 'P&I cover note', 'Last port state control report'], [['purpose', 'Purpose of call', 'الغرض من الرسو', 'select', true, [], 'callPurpose'], ['durationDays', 'Duration (days)', 'المدة (أيام)', 'number', true, []]]],
  ['VESSEL-NOC', 'Vessel Movement No Objection Certificate', 'شهادة عدم ممانعة لحركة السفينة', 1, 'Licensing', 'VESSEL', 'VESSEL_NOC', 500, 5000, 3, true, ['Movement plan'], [['movementType', 'Movement', 'نوع الحركة', 'select', true, [], 'movementType']]],
  ['SEAFARER-COC', 'Certificate of Competency — issue or revalidate', 'شهادة الكفاءة — إصدار أو تجديد', 2, 'Seafarers', 'SEAFARER', 'CERTIFICATE_OF_COMPETENCY', 800, 8000, 14, false, ['Sea service testimonial', 'Medical fitness certificate', 'STCW course certificates', 'Seafarer identity document and passport copy'], [['grade', 'Certificate grade applied for', 'درجة الشهادة المطلوبة', 'select', true, [], 'cocGrade'], ['seaServiceMonths', 'Approved sea service (months)', 'الخدمة البحرية المعتمدة (أشهر)', 'number', true, []]]],
  ['SEAFARER-COP', 'Certificate of Proficiency — issue', 'شهادة الإتقان — إصدار', 2, 'Seafarers', 'SEAFARER', 'CERTIFICATE_OF_PROFICIENCY', 400, 4000, 10, false, ['Course completion certificate', 'Medical fitness certificate'], [['course', 'Course', 'الدورة', 'select', true, [], 'copCourse']]],
  ...
  ['SEAFARER-GMDSS', 'GMDSS Operator Certificate — issue', 'شهادة مشغل GMDSS — إصدار', 2, 'Seafarers', 'SEAFARER', 'GMDSS_CERTIFICATE', 400, 4000, 10, false, ['GMDSS course certificate', 'Medical fitness certificate'], [['grade', 'Certificate', 'الشهادة', 'select', true, [], 'gmdssGrade']]],
  ...
  ['SEAFARER-SEAMAN-CARD', 'Seafarer Identity Card — issue', 'بطاقة هوية البحار — إصدار', 2, 'Seafarers', 'SEAFARER', 'SEAMAN_CARD', 200, 2000, 7, false, ['Passport copy', 'Photograph', 'Sea service testimonial'], [['deliveryOffice', 'Collection office', 'مكتب الاستلام', 'select', true, [], 'deliveryOffice']]],
  ...
  ['SEAFARER-RECORD-BOOK', 'Seafarer record book — issue or replacement', 'سجل خدمة البحار — إصدار أو بدل فاقد', 2, 'Seafarers', 'SEAFARER', '', 200, 2000, 7, false, ['Passport copy', 'Photograph', 'Loss report (replacement only)'], [['reason', 'Reason', 'السبب', 'select', true, [], 'recordBookReason']]],
  ...
  ['INSP-ISM-AUDIT', 'ISM audit request', 'طلب تدقيق ISM', 5, 'Inspection', 'COMPANY', '', 3000, 30000, 14, false, ['Internal audit report', 'Fleet list'], [['auditType', 'Audit', 'نوع التدقيق', 'select', true, [], 'auditType']]],
...
export function buildServiceDefinitions(profile: string): WorldServiceDefinition[] {
  ...
  return rows.map(([code, name, nameAr, domain, category, subjectKind, issuesInstrument, feeAed, feeInr, slaDays, autoApprovable, docs, fields]) => {
    ...
    return {
      ...
      formFields: fields.map(([key, label, labelAr, type, required, options, lookup]) => ({ key, label, labelAr: ae ? labelAr : undefined, type, options, lookup, required, help: '' })),
...
export function buildServiceRequests(rng: Prng, profile: string, defs: WorldServiceDefinition[], licences: WorldLicence[], users: WorldUser[], companies: WorldCompany[], vessels: WorldVessel[], seafarers: WorldSeafarer[], berths: WorldBerth[], now: Date): WorldServiceRequest[] {
  ...
  // a seeded answer to a master-backed select is one of that master's codes, exactly as the runtime would accept
  const masters = buildLookups(profile); const codesOf = (category: string) => masters.filter((l) => l.category === category).map((l) => l.code);
  ...
  for (let k = 0; k < 72; k++) {
    ...
    push(def, kind, subjectId, subjectModel, subjectLabel, status, created, submitted, closed, Object.fromEntries(def.formFields.map((f) => [f.key, f.type === 'number' ? rng.int(1, 12) : f.type === 'select' ? (f.lookup ? codesOf(f.lookup)[0] ?? '' : f.options[0] ?? '') : f.type === 'date' ? iso(now.getTime() + rng.int(1, 30) * D) : 'As stated in the application'])));
  }
