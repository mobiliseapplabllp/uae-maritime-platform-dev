...
describe('permission catalogue', () => {
  ...
  it('seeds nineteen roles whose permissions are all known, thirteen of them system roles', () => {
    ...
    // the agents' identity carries no second factor because it never signs in at all
    expect(ROLE_CATALOGUE.filter((r) => r.mfaRequired === false).map((r) => r.code).sort()).toEqual(['AG', 'AIA', 'MA']);
    expect(ROLE_CATALOGUE.filter((r) => r.system)).toHaveLength(13);
    ...
    expect(counts).toMatchObject({ HM: 27, MS: 26, FO: 16, AG: 12, MA: 10, ND: 14, TS: 8, HO: 9, BC: 7, SO: 6, PP: 5, LO: 11, AP: 10, RS: 23, MV: 17, AIG: 8, IA: 9, AIA: 20 });
    // the agents' identity reads every module and acts only through the desk's reversible verbs
    const aia = ROLE_CATALOGUE.find((r) => r.code === 'AIA')!;
    expect(aia.permissions.filter((x) => !x.endsWith('.view'))).toEqual(['services.assess']);
  });
