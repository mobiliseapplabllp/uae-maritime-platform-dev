...
export const MODULE_SETTING_DEFAULTS: Record<string, Record<string, unknown>> = {
  ...
  inspect:   { findingDueDays: 14, detentionThreshold: 1, passScorePct: 80,
    // the Smart Inspection programme: when it started, how long it runs, and the six 18-month KPI targets measured from the platform's events
    kpiProgrammeStart: '', kpiProgrammeMonths: 18, kpiDossierTargetPct: 100, kpiAiReportTargetPct: 70, kpiNoticeTargetPct: 80, kpiNoticeMinutes: 30,
    kpiPredictionTargetPct: 65, kpiPredictionWindowMonths: 12, kpiReportReductionTargetPct: 50, kpiReportBaselineMinutes: 0, kpiRestrictionTargetPct: 100, kpiRestrictionMinutes: 60 },
  facil:     { auditIntervalMonths: 12, renewalReminderDays: 90 },
  ...
  agents:    { escalationHours: 4, suspensionNoticeHours: 4 },
  /* Users & security — every value here is read by the identity service at the moment it matters: token lifetimes at
   * sign-in and refresh, the idle window at refresh, the MFA deadline at sign-in, the four-eyes list when a role is
   * granted, the dormancy and review cadences when the scheduler fires. */
  admin:     { accessTokenMinutes: 15, refreshTokenHours: 12, idleTimeoutMinutes: 30, passwordMinLength: 12, auditRetentionDays: 1825,
    mfaRequiredFrom: '', mfaGraceDays: 14, dormantAfterDays: 90, dormantAction: 'DEACTIVATE', accessReviewDays: 90,
    fourEyesPermissions: ['*', 'users.manage', 'roles.manage'] },
};

/* Platform-wide settings, one section per concern. Every value here is read by a service at the moment it matters;
 * anything a module owns lives in MODULE_SETTING_DEFAULTS instead, so each setting has exactly one home. */
export const SETTING_SECTIONS = ['org', 'billing', 'notifications', 'smtp', 'ai'] as const;
export type SettingSection = (typeof SETTING_SECTIONS)[number];
/** The keys each section carries. A key outside the list is refused on write and dropped on read, so a retired setting cannot linger. */
export const SETTING_FIELDS: Record<SettingSection, readonly string[]> = {
  org: ['portName', 'operator', 'unlocode', 'jurisdiction', 'address', 'taxId', 'taxIdLabel', 'currency', 'timezone', 'contactEmail', 'contactPhone', 'languages'],
