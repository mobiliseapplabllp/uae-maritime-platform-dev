    ...
    <>
      <CrudPage<UserRow>
        ...
        formFields={(editing) => [
          ...
          ...(!(editing && 'id' in editing) ? [{ name: 'password', label: 'Initial password', required: true, helper: 'Min 12 characters', type: 'password' as const }] : []),
      ...
      <Dialog open={!!resetFor} onClose={() => !busy && setResetFor(null)} maxWidth="xs" fullWidth>
        ...
        <DialogContent sx={{ pt: '12px !important' }}><TextField autoFocus fullWidth type="password" label="New password" value={pwd} onChange={(e) => setPwd(e.target.value)} helperText="Min 12 characters — share it with the user securely" /></DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button color="inherit" onClick={() => setResetFor(null)} disabled={busy}>Cancel</Button>
          <Button variant="contained" disabled={busy || pwd.length < 8} onClick={() => { if (!resetFor) return; setBusy(true); api.post(`/users/${resetFor.id}/reset-password`, { password: pwd }).then(() => { dispatch(notify('Password reset')); setResetFor(null); }).catch((e: Error) => dispatch(notify({ message: e.message, severity: 'error' }))).finally(() => setBusy(false)); }}>Reset</Button>
        </DialogActions>
      </Dialog>
    </>
  );
