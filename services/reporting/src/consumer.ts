...
export const PROJECTIONS: Record<string, (c: PoolClient, e: Row) => Promise<void>> = {
  ...
  invoice: (c, e) => up(c, 'rm_invoices', { due_at: e.dueAt ?? null, paid_amount: e.paidAmount ?? (e.status === 'PAID' ? (e.total ?? 0) : 0), id: e.id, number: e.number, port_call_id: e.portCallId ?? null, vessel_id: e.vesselId ?? null, vessel_name: e.vesselName ?? null, bill_to_name: e.billTo?.name ?? e.billToName ?? null, subtotal: e.subtotal ?? 0, tax_amount: e.taxAmount ?? 0, total: e.total ?? 0, currency: e.currency ?? 'AED', status: e.status, issued_at: e.issuedAt ?? null, paid_at: e.paidAt ?? null, created_at: e.createdAt ?? new Date(), ...sc(e) }, 'number'),
  ...
  metInstitution: (c, e) => up(c, 'rm_met_institutions', { id: e.id, code: e.code, name: e.name, name_ar: e.nameAr ?? null, institution_type: e.institutionType, city: e.city ?? null, status: e.status ?? 'ACTIVE', accreditation_status: e.accreditation?.status ?? e.accreditationStatus ?? 'NONE', accredited_until: e.accreditation?.until ?? e.accreditedUntil ?? null, programmes: e.programmeCount ?? e.programmes?.length ?? 0, approved_programmes: e.approvedProgrammes ?? 0, seats_per_year: e.seatsPerYear ?? 0, instructors: e.instructors ?? 0, ...sc(e) }, 'code'),
  crewList: (c, e) => up(c, 'rm_crew_lists', { id: e.id, number: e.number, vcn: e.vcn ?? null, vessel_id: e.vesselId ?? null, vessel_name: e.vesselName ?? '', movement: e.movement ?? 'ARRIVAL', list_date: e.date ?? e.listDate ?? new Date(), source: e.source, agent_code: e.agentCode ?? null, status: e.status ?? 'RECEIVED', ok: e.ok ?? e.checks?.ok ?? null, row_count: e.rowCount ?? e.rows?.length ?? 0, matched: e.matched ?? 0, foreign_count: e.foreignCount ?? 0, flagged: e.flagged ?? 0, shortfalls: e.checks?.manning?.shortfalls ?? e.shortfalls ?? 0, ...sc(e) }, 'number'),
  foreignSeafarer: (c, e) => up(c, 'rm_foreign_seafarers', { id: e.id, name: e.name, nationality: e.nationality ?? null, id_number: e.idNumber, last_rank: e.lastRank ?? null, appearances: e.appearances ?? 0, status: e.status ?? 'LEDGER', endorsed: !!e.endorsement, last_seen_at: e.lastSeenAt ?? null }),
  serviceRequest: (c, e) => {
    const due = e.slaDueAt ?? e.dueAt ?? null; const decided = e.decidedAt ?? e.decision?.at ?? null;
    const open = ['SUBMITTED', 'UNDER_ASSESSMENT', 'INFO_REQUESTED'].includes(String(e.status));
    return up(c, 'rm_service_requests', {
      id: e.id, number: e.number ?? e.requestNo, definition_key: e.definitionKey ?? e.serviceCode ?? '', definition_name: e.definitionName ?? e.serviceName ?? '', category: e.category ?? null,
      subject_kind: e.subjectKind ?? null, subject_name: e.subjectName ?? e.subjectLabel ?? null, applicant_org: e.applicant?.organisation ?? null, applicant_org_code: e.applicant?.organisationCode ?? null,
      status: e.status, current_state: e.currentState ?? e.currentStage ?? null, sla_due_at: due, sla_breached: !!e.slaBreached || (open && !!due && new Date(due).getTime() < Date.now()),
      submitted_at: e.submittedAt ?? null, decided_at: decided, closed_at: e.closedAt ?? null, fees_total: e.fees?.total ?? e.fee?.amount ?? null,
      payment_status: e.payment?.status ?? (e.fee ? (e.fee.paid ? 'PAID' : e.fee.amount > 0 ? 'DUE' : 'NOT_REQUIRED') : null), created_at: e.createdAt ?? null, ...sc(e),
    }, 'number');
  },
...
const TABLES: Record<string, string> = { serviceRequest: 'rm_service_requests', user: 'rm_users', berth: 'rm_berths', vessel: 'rm_vessels', vesselCertificate: 'rm_vessel_certificates', portCall: 'rm_port_calls', invoice: 'rm_invoices', inspection: 'rm_inspections', incident: 'rm_incidents', seafarer: 'rm_seafarers', company: 'rm_companies', instrument: 'rm_instruments', legalInstrument: 'rm_legal_instruments', registration: 'rm_registrations', tariff: 'rm_tariffs', resource: 'rm_resources', checklistTemplate: 'rm_checklists', agentDecision: 'rm_agent_decisions', metInstitution: 'rm_met_institutions', crewList: 'rm_crew_lists', foreignSeafarer: 'rm_foreign_seafarers' };
