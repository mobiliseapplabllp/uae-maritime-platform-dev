...
const up = async (c: PoolClient, table: string, cols: Record<string, unknown>) => {
...
export const PROJECTIONS: Record<string, (c: PoolClient, e: Row) => Promise<void>> = {
  ...
  berth: (c, e) => up(c, 'rm_berths', { id: e.id, code: e.code, name: e.name, terminal: e.terminal, berth_type: e.berthType, loa_max: e.loaMax ?? null, draft_max: e.draftMax ?? null, status: e.status ?? 'OPERATIONAL', outages: e.outages ?? [] }),
  vessel: (c, e) => up(c, 'rm_vessels', { id: e.id, imo: e.imo, name: e.name, mmsi: e.mmsi ?? null, call_sign: e.callSign ?? null, flag: e.flag ?? null, type: e.type, built: e.built ?? null, dwt: e.dwt ?? null, grt: e.grt ?? null, loa: e.loa ?? null, beam: e.beam ?? null, max_draft: e.maxDraft ?? null, owner: e.owner ?? null, operator: e.operator ?? null, manager: e.manager ?? null, agent_code: e.agentCode ?? e.agent ?? null, agent_name: e.agentName ?? null, class_society: e.classSociety ?? null, teu_capacity: e.teuCapacity ?? null, liner: !!e.liner, real: !!e.real, status: e.status ?? 'ACTIVE', next_dry_dock: e.nextDryDock ?? null, registry_state: e.registry?.state ?? e.registryState ?? 'UNREGISTERED', registry: { ...(e.registry ?? {}), riskScore: e.riskScore ?? e.registry?.riskScore, riskBand: e.riskBand ?? e.registry?.riskBand } }),
  vesselCertificate: (c, e) => up(c, 'rm_vessel_certificates', { id: e.id, vessel_id: e.vesselId, cert_type: e.certType, number: e.number ?? null, issuer: e.issuer ?? null, issue_date: e.issueDate ?? null, expiry_date: e.expiryDate, on_register: !!e.onRegister, in_force: e.inForce ?? true, force_reason: e.forceReason ?? null, signed: !!e.signed }),
  portCall: (c, e) => { const ops: Row[] = e.cargoOps ?? []; return up(c, 'rm_port_calls', { id: e.id, vcn: e.vcn, vessel_id: e.vesselId, vessel_name: e.vesselName, vessel_type: e.vesselType ?? null, agent_code: e.agentCode ?? null, agent_name: e.agentName ?? null, status: e.status, eta: e.eta, etb: e.etb ?? null, etd: e.etd ?? null, ata: e.ata ?? null, atb: e.atb ?? null, atd: e.atd ?? null, berth_id: e.berthId ?? null, berth_code: e.berthCode ?? null, prev_port: e.prevPort ?? null, next_port: e.nextPort ?? null, cargo_ops: ops, cargo_mt: ops.reduce((s, o) => s + Number(o.qtyMT || 0), 0), teu: ops.filter((o) => o.unit === 'TEU').reduce((s, o) => s + Number(o.qty || 0), 0) }); },
  invoice: (c, e) => up(c, 'rm_invoices', { id: e.id, number: e.number, port_call_id: e.portCallId ?? null, vessel_id: e.vesselId ?? null, vessel_name: e.vesselName ?? null, bill_to_name: e.billTo?.name ?? e.billToName ?? null, subtotal: e.subtotal ?? 0, tax_amount: e.taxAmount ?? 0, total: e.total ?? 0, currency: e.currency ?? 'AED', status: e.status, issued_at: e.issuedAt ?? null, paid_at: e.paidAt ?? null, created_at: e.createdAt ?? new Date() }),
  inspection: (c, e) => { const f: Row[] = e.findings ?? []; return up(c, 'rm_inspections', { id: e.id, number: e.number, vessel_id: e.vesselId ?? null, vessel_name: e.vesselName ?? null, type: e.type, inspector: e.inspector ?? null, status: e.status, result: e.result || null, detention: !!e.detention, planned_at: e.plannedAt ?? null, started_at: e.startedAt ?? null, closed_at: e.closedAt ?? null, open_findings: e.openFindings ?? f.filter((x) => x.status === 'OPEN').length, total_findings: e.totalFindings ?? f.length, score_pct: e.scorePct ?? null }); },
  incident: (c, e) => up(c, 'rm_incidents', { id: e.id, number: e.number, title: e.title, category: e.category ?? null, type: e.type, severity: e.severity, priority: e.priority ?? null, status: e.status, vessel_id: e.vesselId ?? null, vessel_name: e.vesselName ?? null, assigned_to_name: (typeof e.assignedTo === 'string' ? e.assignedTo : e.assignedTo?.name) ?? e.assignedToName ?? null, reported_at: e.reportedAt, acknowledged_at: e.acknowledgedAt ?? null, resolved_at: e.resolvedAt ?? null, closed_at: e.closedAt ?? null }),
  ...
  company: (c, e) => up(c, 'rm_companies', { id: e.id, code: e.code, name: e.name, category: e.category ?? null, status: e.status ?? 'ACTIVE', address: e.address ?? null, tax_id: e.taxId ?? e.gstin ?? null }),
  instrument: (c, e) => up(c, 'rm_instruments', { id: e.id, number: e.number ?? e.licenseNo, subject_kind: e.subjectKind ?? 'COMPANY', subject_id: e.subjectId ?? null, entity_name: e.entityName, entity_type: e.entityType, instrument_class: e.instrumentClass ?? 'LICENCE', status: e.status, applied_date: e.appliedDate ?? null, issue_date: e.issueDate ?? null, expiry_date: e.expiryDate ?? null, statutory: !!e.statutory, in_force: e.inForce ?? true, signed: !!(e.signed ?? e.signature?.value), performance_rating: e.performanceRating ?? null, audits: e.audits?.length ?? e.auditsCount ?? 0 }),
  legalInstrument: (c, e) => up(c, 'rm_legal_instruments', { id: e.id, ref_no: e.refNo, title: e.title, type: e.type, status: e.status, issued_date: e.issuedDate ?? null, ack_required: !!e.ackRequired, acknowledged_by: e.acknowledgedBy ?? [] }),
  registration: (c, e) => up(c, 'rm_registrations', { id: e.id, number: e.number ?? e.applicationNo, vessel_id: e.vesselId ?? null, vessel_name: e.vesselName ?? null, kind: e.kind, status: e.status, submitted_at: e.submittedAt ?? null, closed_at: e.closedAt ?? null, due_at: e.dueAt ?? null }),
  tariff: (c, e) => up(c, 'rm_tariffs', { id: e.id, code: e.code, name: e.name, category: e.category ?? 'MARINE', unit: e.unit, rate: e.rate, active: e.active ?? true, revisions: e.revisions ?? [] }),
  resource: (c, e) => up(c, 'rm_resources', { id: e.id, code: e.code, name: e.name, type: e.type, status: e.status ?? 'AVAILABLE', jobs: e.jobs ?? [], outages: e.outages ?? [] }),
