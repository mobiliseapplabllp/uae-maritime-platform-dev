...
export async function seedMdm(databaseUrl: string, profile?: string) {
  ...
  const counts = await withTx(pool, async (c) => {
    ...
    for (const s of world.settings) await c.query('INSERT INTO settings(key, value, updated_by) VALUES ($1, $2, $3) ON CONFLICT (key) DO NOTHING', [s.key, JSON.stringify(s.value), 'seed']);
