...
import { KIT_ENV, KIT_POOL, AuditClient, RequirePerm, ServiceOnly, zod, paged, parsePage, escapeLike, notFound, withTx, enqueue, eventFromContext } from '@maritime/service-kit';
...
export class CompaniesController {
  ...
  async list(@Query() query: PageQuery & { category?: string; status?: string }) {
  ...
  async get(@Param('id') id: string) { const r = await this.pool.query<Row>('SELECT * FROM companies WHERE id = $1 OR code = $1', [id]); if (!r.rows[0]) throw notFound('Company not found'); return toApi(r.rows[0]); }
