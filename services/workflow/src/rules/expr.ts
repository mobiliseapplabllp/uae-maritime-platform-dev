/* In-process copy of the rules service evaluator (services/rules/src/expr.ts) so guards and fee lines evaluate without a network hop
 * when RULES_MODE=inline. Keep the two files identical; the rules service is the source of truth. */
