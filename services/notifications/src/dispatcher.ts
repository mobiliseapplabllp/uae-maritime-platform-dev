...
export const RULES: Array<{ type: string; audiencePerm: string; severity: string; /** When given, the rule fires only for events it answers true for. */ when?: (d: Record<string, unknown>) => boolean; title: (d: Record<string, unknown>) => string; body?: (d: Record<string, unknown>) => string; link?: (d: Record<string, unknown>) => string }> = [
  ...
  // the federal security review of a port facility: sent, cleared, or sent back with conditions
  { type: EVENTS.facilities.facilityReviewChanged, audiencePerm: 'facilities.view', severity: 'info', when: (d) => d.reviewFrom == null, title: (d) => `Security review submitted — ${d.name} (${d.reference})`, body: (d) => `${d.requestedBy}: ${d.reason}`, link: (d) => `/port-facilities/${d.facilityId}` },
  { type: EVENTS.facilities.facilityReviewChanged, audiencePerm: 'facilities.view', severity: 'success', when: (d) => d.reviewFrom != null && d.reviewStatus === 'CLEARED', title: (d) => `Security review cleared — ${d.name}`, body: (d) => `${d.reference}${d.pushed ? ', reported by the authority' : ''}`, link: (d) => `/port-facilities/${d.facilityId}` },
  { type: EVENTS.facilities.facilityReviewChanged, audiencePerm: 'facilities.view', severity: 'error', when: (d) => d.reviewFrom != null && d.reviewStatus === 'REJECTED', title: (d) => `Security review rejected — ${d.name}`, body: (d) => (Array.isArray(d.conditions) && d.conditions.length ? `${d.reference}: ${d.conditions.join('; ')}` : String(d.reference)), link: (d) => `/port-facilities/${d.facilityId}` },
